...
void TestVrlimi128Translation()
{
    TestRecompiler recomp;
    RecompilerLocalVariables vars{};
    auto swt = recomp.config.switchTables.end();
    CSRState csr = CSRState::Unknown;

    ppc_insn insn{};
    for (int i = 0; i < powerpc_num_opcodes; ++i)
    {
        if (powerpc_opcodes[i].id == PPC_INST_VRLIMI128)
        {
            insn.opcode = &powerpc_opcodes[i];
            break;
        }
    }
    ...
    insn.operands[1] = 1;
    insn.operands[2] = 4;
    insn.operands[3] = 0xF0;

    Function fn(0, 4);
    bool ok = recomp.Recompile(fn, 0, insn, nullptr, swt, vars, csr);
    fmt::println("vrlimi128 translation {}", ok ? "succeeded" : "failed");
}
