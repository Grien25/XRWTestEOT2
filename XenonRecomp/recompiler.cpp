...
*/
    ...
    case PPC_INST_VRLIMI128:
        printSetFlushMode(true);
        println("\tuint32_t sh = {} & 31;", insn.operands[2]);
        println("\tfor(int i=0;i<4;i++) {{");
        println("\t\tuint32_t val = {}.u32[i];", v(insn.operands[1]));
        println("\t\tval = (val << sh) | (val >> (32 - sh));");
        println("\t\tif (({} >> i) & 1) {{", insn.operands[3]);
        println("\t\t\t{}.u32[i] = val;", v(insn.operands[0]));
        println("\t\t}}");
        println("\t}}");
        break;
